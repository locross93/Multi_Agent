 # statistical_analysis_with_discussion.py
import json
import os
import pandas as pd
import numpy as np
...
def load_data(results_dir):
    ...
    conditions = ['basic', 'gossip', 'gossip-with-ostracism', 'gossip-with-ostracism-discussion']
    data = {}
    ...
    for condition in conditions:
        ...
        # Special case for the discussion condition which has a different file pattern
        if condition == 'gossip-with-ostracism-discussion':
            result_files = list(pathlib.Path(results_dir).glob(f'gossip_exp_*_discussion-gossip-with-ostracism.json'))
        else:
...
def test_between_conditions(df):
    ...
    # 3. Special focus on gossip-with-ostracism vs gossip-with-ostracism-discussion
    if 'gossip-with-ostracism' in wide_data.columns and 'gossip-with-ostracism-discussion' in wide_data.columns:
        print("\nSpecial Comparison: gossip-with-ostracism vs gossip-with-ostracism-discussion")
        t_stat, p_val = stats.ttest_rel(
            wide_data['gossip-with-ostracism'], 
            wide_data['gossip-with-ostracism-discussion']
        )
        df_spec = len(wide_data['gossip-with-ostracism']) - 1
        f_spec = t_stat**2
        eta_sq_spec = f_spec / (f_spec + df_spec)
        
        print(f"F(1, {df_spec}) = {f_spec:.2f}, p = {p_val:.6f}, η² = {eta_sq_spec:.2f}")
        print(f"t({df_spec}) = {t_stat:.2f}, p = {p_val:.6f}")
        
        # Calculate means for verbal description
        mean_ostracism = wide_data['gossip-with-ostracism'].mean()
        mean_discussion = wide_data['gossip-with-ostracism-discussion'].mean()
        diff = mean_discussion - mean_ostracism
        
        if p_val < 0.05:
            direction = "higher" if diff > 0 else "lower"
            print(f"The 'gossip-with-ostracism-discussion' condition showed significantly {direction} contributions")
            print(f"compared to the 'gossip-with-ostracism' condition (difference of {abs(diff):.2f} points).")
        else:
            print(f"There was no significant difference between the conditions (difference of {abs(diff):.2f} points).")
...
def test_linear_trends(df):
    ...
    # Special focus on gossip-with-ostracism vs gossip-with-ostracism-discussion trends
    if 'gossip-with-ostracism' in results and 'gossip-with-ostracism-discussion' in results:
        print("\nSpecial Trend Comparison: gossip-with-ostracism vs gossip-with-ostracism-discussion")
        t_stat = (results['gossip-with-ostracism']['slope'] - results['gossip-with-ostracism-discussion']['slope']) / np.sqrt(
            results['gossip-with-ostracism']['std_err']**2 + results['gossip-with-ostracism-discussion']['std_err']**2)
        dof = 2 * (6 - 2)  # 2 * (n - 2)
        p_val = 2 * (1 - stats.t.cdf(abs(t_stat), dof))
        f_comp = t_stat**2
        eta_sq_comp = t_stat**2 / (t_stat**2 + dof)
        ...
        # Calculate slopes for verbal description
        slope_ostracism = results['gossip-with-ostracism']['slope']
        slope_discussion = results['gossip-with-ostracism-discussion']['slope']
        
        if p_val < 0.05:
            if abs(slope_discussion) > abs(slope_ostracism):
                if slope_discussion > 0:
                    print("The discussion condition shows a significantly steeper increasing trend in contributions.")
                else:
                    print("The discussion condition shows a significantly steeper decreasing trend in contributions.")
            else:
                if slope_ostracism > 0:
                    print("The standard ostracism condition shows a significantly steeper increasing trend in contributions.")
                else:
                    print("The standard ostracism condition shows a significantly steeper decreasing trend in contributions.")
        else:
            print("There is no significant difference in the linear trends between the two conditions.")
...
def plot_contributions_by_round(round_means, results_dir):
    ...
    styles = {
        ...
        'gossip-with-ostracism-discussion': {'color': 'purple', 'marker': 'o', 'linestyle': '--', 'label': 'Gossip with Ostracism and Discussion'}
    }
    
    for condition in conditions:
        condition_data = round_means[round_means['condition'] == condition]
        plt.plot(condition_data['round'], condition_data['contribution'], 
                **styles[condition], linewidth=2, markersize=8)
    ...
    plt.savefig(os.path.join(results_dir, 'contributions_by_condition_with_discussion2.png'), dpi=300)
    plt.close()
...
def main(results_dir):
    ...
    print("\nAnalysis complete. Figure saved as 'contributions_by_condition_with_discussion.png'")
